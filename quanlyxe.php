...
<?php
    ...
    if(isset($_POST['sbm'])){
        ...
        $sql = "INSERT INTO xegiamgia (anh, tenxe, giathue, tinhtrang)
        VALUES ('$image', '$tenxe', '$giathue', '$tinhtrang')";
        $query = mysqli_query($conn, $sql);
        move_uploaded_file($image_tmp, 'C:/xampp/htdocs/PROJECT-BOOKING/img/xegiamgia/'.$image);

    }
?>

<?php
    $sql = "SELECT * FROM xegiamgia ORDER BY ID DESC";
    $query = mysqli_query($conn, $sql);
?>
<div class="content">
    <div class="card">
                ...
                <tbody>
                    ...
                    while ($row = mysqli_fetch_assoc($query)){ ?>
                        <tr>
                            ...
                            <td> <a href="edit.php?page_layout=edit&id=<?php echo $row['id']; ?>">Sửa</a> </td>
                            <td> <a onclick="return Del()" href="delete.php?page_layout=delete&id=<?php echo $row['id']; ?>">Xóa</td>
                        </tr>
                    <?php }
                    ?> 
                </tbody>
